...
The consent script POSTs via `fetch()`, which follows the 302 to the callback; the final URL (`response.url`) is passed to native through the `llmdash-oauth://callback` scheme. Native fetches it exactly once (the server's one-time `state` is consumed by a single request), extracts the JWT straight from the callback HTML, writes it into the same `CapacitorStorage` prefs that `storage.ts` reads, and returns to the bundled app UI. This avoids the fragile alternative — letting the callback page's JS store the JWT in `localStorage` and redirect to `/`, where the web SPA boots with still-empty native prefs and shows the web login page instead of handing the token to the app.
