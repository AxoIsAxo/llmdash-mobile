...
Each flow starts from a clean session: before opening the authorize URL, native code expires the WebView's cookies for the Extrovert provider and the LLMDash server, so retries never carry a stale provider session. The provider's consent page renders its hidden `_csrf` field empty on a fresh session (its CSRF token is generated lazily and the session is regenerated at login), and its CSRF check only parses urlencoded bodies.

All OAuth HTTP happens **natively**, not in the WebView: the injected consent script only intercepts the submit and hands the form fields + clicked button to native via the `llmdash-oauth://consent` scheme. Native mints the CSRF token (GETs the provider homepage with the WebView's cookies), POSTs the form `application/x-www-form-urlencoded` (the format the provider's CSRF check reads), follows the 302 to the callback, extracts the JWT from the HTML, stores it in the same `CapacitorStorage` prefs that `storage.ts` reads, and returns to the bundled app UI. This avoids every in-page pitfall — CSP-blocked fetches, opaque redirect responses, CORS credential mismatches, multipart CSRF being ignored — and the web SPA never boots inside the flow.
