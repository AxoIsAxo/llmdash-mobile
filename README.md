...
Each flow starts from a clean session: before opening the authorize URL, native code expires the WebView's cookies for the Extrovert provider and the LLMDash server, so retries never carry a stale provider session. On the provider's consent page the app also injects a small script that fetches the provider's homepage to obtain the session's CSRF token and fills the consent form's hidden `_csrf` field before submit — the provider renders that field empty on fresh sessions (its CSRF token is generated lazily and the session is regenerated at login), which otherwise makes the Authorize POST fail with `CSRF token missing or invalid. Re-open the authorization request.` The consent form's buttons are disabled on first submit so a double-click can't issue two authorization codes.

The server callback is **fetched natively** instead of rendered: `shouldInterceptRequest` catches the callback URL before the WebView sends it (the server's one-time `state` is consumed by exactly one request), the JWT is extracted from the callback HTML, written into the same `CapacitorStorage` prefs that `storage.ts` reads, and the WebView returns to the bundled app UI. This avoids the fragile alternative — letting the callback page's JS store the JWT in `localStorage` and redirect to `/`, where the web SPA boots with still-empty native prefs and shows the web login page.
