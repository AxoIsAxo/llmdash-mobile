...
import {
  ...
  ArrowUp, ArrowDown, Upload, File, Eye, Mic, Sparkles, GitBranch, Lock, Unlock, Activity
} from 'lucide-react'
import { api } from '../api'
import type { User, ProviderConfig, ScannedProvider, ModelConfig, SubscriptionPlan, PlanModelLimit, UserSubscription as UserSub, GitRepo, GitInfo, GitAuditEntry } from '../types'
...
type AdminTab = 'users' | 'providers' | 'models' | 'apikeys' | 'subscriptions' | 'uploads' | 'git'
      ...
      <div className="llm-modal bg-theme-bg-secondary rounded-2xl w-full max-w-3xl max-h-[85vh] flex flex-col border border-theme-border-light" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-theme-border shrink-0">
          {[
            ...
            { key: 'git' as AdminTab, icon: GitBranch, label: 'Git' },
          ].map(t => (
            <button
              key={t.key}
...
function GitTab() {
  ...
  const [form, setForm] = useState({
    ...
    credential: '', default_branch: 'main', pr_preferred: true,
  })
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setForm({ name: '', clone_url: '', access: 'read', auth_type: 'none', credential: '', default_branch: 'main', pr_preferred: true })
      setShowForm(false)
      await load()
    } catch (e: any) { setError(e.message) }
    setSaving(false)
  ...
  const handleDelete = async (repo: GitRepo) => {
    if (!window.confirm(`Remove '${repo.name}' from the git allowlist?`)) return
    setError('')
    try {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-theme-muted mb-3">
          The AI can clone, commit, and push only in repositories on this allowlist. Every commit is authored by{' '}
          <span className="text-theme-accent-text">{info ? `${info.bot_name} <${info.bot_email}>` : 'the bot identity'}</span> —
          never the user's git identity. Read-only by default; grant write access per repo. All actions are audit-logged;
          any user chatting with the bot can direct it to use write-enabled repos. Customize the author via{' '}
          <code className="text-theme-accent-text/80">GIT_BOT_NAME</code> / <code className="text-theme-accent-text/80">GIT_BOT_EMAIL</code> in the API Keys tab.
        </p>
